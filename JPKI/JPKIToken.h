...
#ifndef _JPKITOKEN_H_
#define _JPKITOKEN_H_
...
// "The" token
//
class JPKIToken : public Tokend::ISO7816Token
{
	NOCOPY(JPKIToken)
public:
	JPKIToken();
	...
	uint32_t selectApplet();
	void select(uint8_t shortEFName);
	void sign(const uint8_t *toSign, size_t toSignLength, uint8_t *signature);
	void readCertificate(uint8_t shortEFName, uint8_t *result,
		size_t &resultLength);
...
public:
	bool mAppletSelected;
	uint8_t mCurrentShortEF;
	uint32_t mPinStatus;
...
/* arch-tag: 70E0814E-1C88-11D9-8638-000A9595DEEE */
